...
class Mail extends Component {
    ...
    public function _init($config) {
		...
		if(isset($config['reply']) && isset($config['reply']['address'])) {
			$name = isset($config['reply']['name']) ? $config['reply']['name'] : null;
    ...
    private function _getHeaders() {
        ...
        if(!empty($this->_attachments)) {
            $header.= 'Content-Type: multipart/mixed; boundary="'.$this->_boundary.'"'. self::MAIL_EOF;
        } else {
            $header.= 'Content-type: text/html; charset=iso-8859-1'.self::MAIL_EOF;
        }
    }
    ...
    private function _getBody($message) {
        ...
        if(!empty($this->_attachments)) {
            $body.= "--".$this->_boundary.self::MAIL_EOF ;
            $body.= "Content-Type: text/html; charset=\"iso-8859-1\"".self::MAIL_EOF;
            $body.= "Content-Transfer-Encoding: 7bit". self::MAIL_EOF.self::MAIL_EOF;
        }
    ...
    private function _getBodyAttachements() {
        if(!empty($this->_attachments)) {
            ...
            foreach($this->_attachments as $file) {
                $body.= '--'.$this->_boundary.self::MAIL_EOF;
                $body.= 'Content-Type: $mimeType; name="'.basename($file).'"'.self::MAIL_EOF;
    ...
    private function _isMail($address) {
        return preg_match('/^[a-z0-9\'\.\-_\+]+@[a-z0-9\-]+\.([a-z0-9\-]+\.)*+[a-z]{2}/is', $address);
    }
